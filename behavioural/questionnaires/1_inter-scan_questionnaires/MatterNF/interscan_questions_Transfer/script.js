...
form2.addEventListener('input', () => {
    ...
    const radioButtons = form2.querySelectorAll(`input[type="radio"][name^="run"]`);
...
form3.addEventListener('input', () => {
    ...
    if (answer1.value.trim() !== '' && answer2.value.trim() !== '' && answer3.value.trim() !== '' && allRated) {
...
function createQuestionnaire(instanceId) {
    ...
    heading.textContent = `Transfer run ${instanceId}`;
    ...
    questions.forEach((question, questionIndex) => {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            radio.type = 'radio';
            radio.id = `run${instanceId}-q${questionIndex+1}-${i}`;
            radio.name = `run${instanceId}-q${questionIndex+1}`;
            radio.value = i;

            const label = document.createElement('label');
            label.htmlFor = `run${instanceId}-q${questionIndex+1}-${i}`;
            label.textContent = i;
...
function saveRatings() { // finishing saving downloading the data
    ...
    let csvContent =  'TransferRun,Question,Score\n';

    const nrNFRuns = document.getElementById('NrNFRuns').value;

    for (let i = 1; i <= nrNFRuns; i++) {
        for (let q_idx = 1; q_idx <= 2; q_idx++) {

            const radios = document.getElementsByName(`run${i}-q${q_idx}`);
            ...
            csvContent += `run${i},q${q_idx},${selectedRating}\n`;
        };    
    };
    ...
    const filename = `interscan-Transfer-ratings_${subjectID}_ses-0${sessionNumber}_${timestamp}.csv`;
...
function saveStrategyInfo() {
    ...
    data['Question 3: Could you engage with certain emotions more effectively than others?'] = answer3;
