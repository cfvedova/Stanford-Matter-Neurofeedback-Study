...
The images order for the localiser and the NF runs should also be provided in the ./data/subXX/sesX folder.

The ./data/subXX/sesX should contain:

Stock_images.txt (optional)
Localiser1_group.txt (optional)
Localiser2_group.txt (optional)
/run1/
    runNF1_group.txt
/run2/
    runNF2_group.txt
...
Please write _interrupted at the end of the log file. 
...
-------------------------------------------------------------------
    ...
    SHOW_TARGET_LEVEL = True # whether to show arrows for the target level
    ...
    hdr, prt_data = prt.read_prt(wdir + '/prt/Neurofeedback_stimPC.prt')
...
# HOST = "192.168.173.3"
# HOST = "192.168.1.95"
#HOST = "10.8.76.21"  # localhost for testing
HOST = 'localhost'
PORT = 55555
...
hdr, prt_data = prt.read_prt(f'{wdir}/data/{sub}/ses{session}/runNF{run}/runNF{run}_stock.prt')
...
image_file = glob.glob(f'{wdir}/data/{sub}/ses{session}/runNF{run}/runNF{run}*.txt')[0]
...
outdir = f'{wdir}/data/{sub}/ses{session}/runNF{run}'
...
start_time = 0
values_request_time = np.array(FEEDBACK_TIMINGS[0]) - 3 # ask for the deternded time series 3 time point before the feedback since we are asking TBV values with some delay this is just for safety
...
outfile = f'{wdir}/data/{sub}/ses{session}/runNF{run}/outdata_{timestamp}.pkl'
