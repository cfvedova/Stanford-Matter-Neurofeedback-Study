...
Post-scan Stock images ratings
...
image_dir = '/Users/brainvoyager/Desktop/Matter_Neurofeedback/code/post-scan_ratings/stock_images'

sub_img_file = f'/Users/brainvoyager/Desktop/Matter_Neurofeedback/behavioural/initial_ratings/{sub}/stock.txt'
...
if os.path.exists(f'{sub_dir}/{sub}_ses-0{ses}_ratings_stock.txt'):
    print(f'{sub_dir}/{sub}_ses-0{ses}_ratings_stock.txt already exist! Check that the current session number is correct!')
    sys.exit()
...
# Save the data
file_path = os.path.join(sub_dir, f'{sub}_ses-0{ses}_ratings_stock.txt')
data.to_csv(file_path, index=None, sep=' ', mode='a')

#data.to_csv(os.path.join(wdir, 'outdata/'+sub +'/' +sub+'_ratings_stock.txt'), index=None, sep=' ', mode='a')
