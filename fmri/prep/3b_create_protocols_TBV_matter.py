...
emo = ['RECOGNITION', 'FAMILY LOVE', 'FRIENDSHIP', 'PLEASURE', 'Neutral']
...
for run, localiser_order in enumerate(localiser_files):
    ...
    with open(localiser_order, 'r') as f:
        ...
        for line in f.readlines():
            ...
            if 'neutral' in line:
                ...
                emo_order.append('Neutral')
            else:
                localiser_conditions.append('Positive')
                emo_order.append(
                    images_emo_info['Peak Labels'][images_emo_info['Image ID'] == line[:-5].split('/')[-1]].values[0])

    emo_first_occurrence_order = sorted(set(emo_order), key=emo_order.index)
    first_occurence_indices = [emo.index(emo_sort) for emo_sort in emo_first_occurrence_order]
    print('Localiser condition order:')
    print(emo_first_occurrence_order)
    ...
    condition_names = ['Rest'] +emo_first_occurrence_order

    # create template prt
    prt_hdr, prt_data_temp = create_prt()
    prt_hdr['Experiment'] = 'Localiser'
    prt_hdr['NrOfConditions'] = 6

    for n, condition in enumerate(['Rest', 'Positive','Neutral']):
        ...
        if condition == 'Rest':
            ...
            for i in range(N_BLOCKS):
                temp_rest['Start'] += [INITIAL_REST_DURATION + RECALL_DURATION + (BLOCK_DURATION) * i + 1]
                temp_rest['Stop'] += [INITIAL_REST_DURATION + RECALL_DURATION + (BLOCK_DURATION) * i + REST_DURATION]
            ...
            for i in idx_neu:
                if i == 0:
                    temp_neutral['Start'] += [INITIAL_REST_DURATION + 1]
                    temp_neutral['Stop'] += [INITIAL_REST_DURATION + RECALL_DURATION]
                else:
                    temp_neutral['Start'] += [INITIAL_REST_DURATION + i * BLOCK_DURATION + 1]
                    temp_neutral['Stop'] += [INITIAL_REST_DURATION + i * BLOCK_DURATION + RECALL_DURATION]
        else:
            ...
            for i in idx_pos:
                if i == 0:
                    temp_positive['Start'] += [INITIAL_REST_DURATION + 1]
                    temp_positive['Stop'] += [INITIAL_REST_DURATION + RECALL_DURATION]
                else:
                    temp_positive['Start'] += [INITIAL_REST_DURATION + i * BLOCK_DURATION + 1]
                    temp_positive['Stop'] += [INITIAL_REST_DURATION + i * BLOCK_DURATION + RECALL_DURATION]
    ...
    for i in range(len(condition_names)):
        ...
        if condition_names[i] == 'Rest':
            temp_data['NrOfOccurances'] = len(temp_rest['Start'])
            temp_data['Time start'] = temp_rest['Start']
            temp_data['Time stop'] = temp_rest['Stop']
            temp_data['Color'] = colors[i]

        elif condition_names[i] == 'Neutral':
            temp_data['NrOfOccurances'] = len(temp_neutral['Start'])
            temp_data['Time start'] = temp_neutral['Start']
            temp_data['Time stop'] = temp_neutral['Stop']
            temp_data['Color'] = colors[i]

        else:
            # check index for the current emotion and corresponding timings in temp_recall
            # remove control from order
            emo_order_noControl = [item for item in emo_order if item != 'Neutral']
            emo_index = [idx for idx, item in enumerate(emo_order_noControl) if item == condition_names[i]]
            temp_data['NrOfOccurances'] = len(emo_index)
            temp_data['Time start'] = [temp_positive['Start'][idx] for idx in np.array(emo_index)]
            temp_data['Time stop'] = [temp_positive['Stop'][idx] for idx in np.array(emo_index)]
            temp_data['Color'] = colors[i]
